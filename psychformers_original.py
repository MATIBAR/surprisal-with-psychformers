...
# ITALIAN
# C:\Users\Dell\Desktop\Programmi\Surprisal\LangLearn_Training_Data\CItA\texts\texts_stimuli_all\files_list.txt
# C:\Users\Dell\Desktop\Programmi\Surprisal\PsychFormers\output\italian

# SPANISH
# C:\Users\Dell\Desktop\Programmi\Surprisal\LangLearn_Training_Data\COWS-L2H\texts\texts_stimuli_all\files_list.txt
# C:\Users\Dell\Desktop\Programmi\Surprisal\PsychFormers\output\spanish

# python psychformers.py --stimuli_list testo_prova_1.txt --output_directory Output --model ai-forever/mGPT --task surprisal --following_context
...
def process_args(args):
    ...
    if args.model_list:
        try:
            ...
            with open(args.model_list, "r", encoding= "utf-8") as f:
    ...
    if args.task_list:
        try:
            ...
            print(
                "Error: 'metric_list' argument does not have a valid path. Trying to use individual specified metric.")
            try:
                assert args.task
    ...
    if args.stimuli_list:
        try:
            ...
            with open(args.stimuli_list, "r", encoding= "utf-8") as f:
    ...
    return (
    output_directory, primary_decoder, include_following_context, model_list, metric_list, stimulus_file_list, cpu)


def create_and_run_models(model_list, stimulus_file_list, metric_list, primary_decoder, output_directory,
        ...
        for model_name in model_list:
            ...
            try:
                ...
                try:
                    ...
                    print(
                        "Model {0} is not a masked or causal language model. This is not supported".format(model_name))
            try:
                assert model and tokenizer
                if model and tokenizer:
                    try:
                        if model_type == "causal":
                            process_stims_causal(model.to("cuda" if (torch.cuda.is_available() and not cpu) else "cpu"),
                                                 tokenizer, stimulus_file_list, metric_list, model_name_cleaned,
                                                 output_directory, include_following_context)
                        elif model_type == "masked":
                            process_stims_masked(model.to("cuda" if (torch.cuda.is_available() and not cpu) else "cpu"),
                                                 tokenizer, stimulus_file_list, metric_list, model_name_cleaned,
                                                 output_directory, include_following_context)
                    except:
                        print("Cannot run either a masked or causal form of {0}".format(model_name))
            except:
                print("Cannot run experiment without both a tokenizer for and a causal or masked form of {0}".format(
                    model_name))
        ...
        for model_name in model_list:
            ...
            try:
                ...
                try:
                    ...
                    print(
                        "Model {0} is not a causal or masked language model. This is not supported".format(model_name))
            try:
                assert model and tokenizer
                if model and tokenizer:
                    try:
                        if model_type == "causal":
                            ...
                            process_stims_causal_mask(
                                model.to("cuda" if (torch.cuda.is_available() and not cpu) else "cpu"), tokenizer,
                                stimulus_file_list, metric_list, model_name_cleaned, output_directory,
                                include_following_context)
...
def process_stims_causal(model, tokenizer, stimulus_file_list, metric_list, model_name_cleaned, output_directory,
                         include_following_context):
    for i in range(len(stimulus_file_list)):
        ...
        with open(stimulus_file_list[i], 'r', encoding= "utf-8") as f:
            ...
            try:
                ...
                if (len(tokenizer.tokenize("a[!StimulusMarker!]")) == 2):
                    dummy_var_idxs = np.where(
                        (np.array(encoded_stimulus) == tokenizer.encode("[!StimulusMarker!]")[-1]) | (
                                    np.array(encoded_stimulus) == tokenizer.encode("a[!StimulusMarker!]")[-1]))[0]
                    preceding_context = encoded_stimulus[:dummy_var_idxs[0]]
                    if (len(preceding_context) == 0) or (not ((preceding_context[0] == tokenizer.bos_token_id) or (
                            preceding_context[0] == tokenizer.eos_token_id))):
                    ...
                    if "surprisal" in metric_list:
                        get_surprisal_causal(model, tokenizer, preceding_context, following_words, target_words,
                                             stimuli_name, model_name_cleaned, output_directory, stimulus)
            except:
                print("Problem with stimulus on line {0}: {1}\n".format(str(j + 1), stimulus_list[j]))


def get_surprisal_causal(model, tokenizer, preceding_context, following_words, target_words, stimuli_name,
                         model_name_cleaned, output_directory, stimulus):
    filename = output_directory + "/" + stimuli_name + "." + "surprisal" + "." + model_name_cleaned + ".causal.output"
...
def process_stims_masked(model, tokenizer, stimulus_file_list, metric_list, model_name_cleaned, output_directory,
                         include_following_context):
    for i in range(len(stimulus_file_list)):
        stimuli_name = stimulus_file_list[i].split('/')[-1].split('.')[0]
        ...
        for j in range(len(stimulus_list)):
            try:
                stimulus = stimulus_list[j]
                stimulus_spaces = stimulus.replace("*", "[!StimulusMarker!]")
                if (tokenizer.tokenize(" a")[0][0] == tokenizer.tokenize(" b")[0][0]) and (
                        tokenizer.tokenize("a")[0][0] != tokenizer.tokenize("b")[0][0]):
                    stimulus_spaces = stimulus_spaces.replace(" [!StimulusMarker!]", "[!StimulusMarker!] ")
                else:
                    stimulus_spaces = stimulus_spaces.replace("[!StimulusMarker!]", "[!StimulusMarker!] ")
                    stimulus_spaces = stimulus_spaces.replace(" [!StimulusMarker!]", "[!StimulusMarker!]")
                ...
                if (len(tokenizer.tokenize("a[!StimulusMarker!]")) == 2):
                    dummy_var_idxs = np.where(
                        (np.array(encoded_stimulus) == tokenizer.encode("[!StimulusMarker!]")[-2]) | (
                                    np.array(encoded_stimulus) == tokenizer.encode("a[!StimulusMarker!]")[-2]))[0]
                    preceding_context = encoded_stimulus[:dummy_var_idxs[0]]
                    ...
                    following_words = encoded_stimulus[dummy_var_idxs[1] + 1:]
                    if "surprisal" in metric_list:
                        get_surprisal_masked(model, tokenizer, preceding_context, following_words, target_words,
                                             stimuli_name, model_name_cleaned, output_directory,
                                             include_following_context, stimulus)
            except:
                print("Problem with stimulus on line {0}: {1}\n".format(str(j + 1), stimulus_list[j]))


def get_surprisal_masked(model, tokenizer, preceding_context, following_words, target_words, stimuli_name,
                         model_name_cleaned, output_directory, include_following_context, stimulus):
    filename = output_directory + "/" + stimuli_name + "." + "surprisal" + "." + model_name_cleaned + ".masked.output"
    current_context = copy.deepcopy(preceding_context)
    all_probabilities = []
    for i in range(len(target_words)):
...
def process_stims_causal_mask(model, tokenizer, stimulus_file_list, metric_list, model_name_cleaned, output_directory,
                              include_following_context):
    for i in range(len(stimulus_file_list)):
        stimuli_name = stimulus_file_list[i].split('/')[-1].split('.')[0]
        ...
        for j in range(len(stimulus_list)):
            try:
                ...
                if (len(tokenizer.tokenize("a[!StimulusMarker!]")) == 2):
                    ...
                    if "surprisal" in metric_list:
                        get_surprisal_causal_mask(model, tokenizer, preceding_context, following_words, target_words,
                                                  stimuli_name, model_name_cleaned, output_directory,
                                                  include_following_context, stimulus)
            except:
                print("Problem with stimulus on line {0}: {1}\n".format(str(j + 1), stimulus_list[j]))
...
def get_surprisal_causal_mask(model, tokenizer, preceding_context, following_words, target_words, stimuli_name,
                              model_name_cleaned, output_directory, include_following_context, stimulus):
    filename = output_directory + "/" + stimuli_name + "." + "surprisal" + "." + model_name_cleaned + ".causal_mask.output"
    current_context = copy.deepcopy(preceding_context)
    all_probabilities = []
